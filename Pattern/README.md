...
## 단계는 얼마나 커야할까?
이 질문은 다음 두가지를 포함한다.
1. 각 테스트가 다뤄야 할 범위가 얼마나 넓은가?
2. 리팩토링을 하면서 얼마나 많은 중간 단계를 거쳐야 하는가?

- 리팩토링 초기에는 아주 작은 단계로 작업할 준비가 되어야한다.
- 초기에는 작은 단계로 빈번하게 리팩토링이 수정될 것이다.
- 그리고 조금씩 몇단계를 건너뛰는 작업을 진행하며 리팩토링을 진행한다.
- 앞서 말했듯이 초기에는 리팩토링의 수정이 빈번한데 이때 IDE의 리팩토링 툴을 사용하면 간편.
...
- 연산자
- 다형성
- 단, 내가 작성하는 것들에 대해서만 테스트 하자

## 문제가 있음을 알려주는 테스트의 속성

- 긴 셋업 코드
  - 하나의 assertion을 수행하기 위해 길고 복잡한 객체 생성코드가 나타난다면, 나눌 필요가 있는 것
- 셋업 중복
  - 공통의 셋업 코드를 만들기 어렵다면, 서로 엉킨 객체들이 너무 많다는 것
- 실행 시간이 오래 걸리는 테스트
  - 자주 실행되지 않기에 자주 동작시키지 않는다
  - 즉, 어플리케이션의 작은 부분만 따로 테스트하기 힘들다는 것
  - 설계 문제를 의미하고 설꼐를 적절히 변경해줄 필요가 있다
- 깨지기 쉬운 테스트
  - 예상치 못하게 실패하는 테스트가 있다면, 다른 부분에 이상한 방법으로 영향을 끼친다는 뜻
  - 연결을 끊거나 두부분을 합해야한다.

## 테스트를 지워야할 때는 언제인가

서로 겹치는 두개의 테스트가 있다면 지우는 기준은 무엇인가?
- 첫째 기준은 자신감
  - 하나를 지워도 프로그램에 영향을 끼치지 않는다는 자신감이 있다면 지워도 된다
- 둘째 기준은 커뮤니케이션
  - 두 개의 테스트가 동일한 부분을 실행하더라도, 서로 다른 시나리오를 설명한다면 나둬야한다.

## 프로젝트 중반 TDD를 도입하려면 어떻게 할까?

테스트를 염두에 두지 않고 만든 코드는 테스트하기가 그리 쉽지 않다.
- 일부만 격리해서 실행하고 결과를 확인하기가 쉽지 않다.
- 리팩토링 과정에서의 에러와 다른 코드가 문제인지 테스트가 없기에 알아낼 수 없다.

어떻게 해야될지 모호하지만, 확실히 하지 말아야할 일은 있다.
- 코드 전체를 위한 테스트를 한번에 다 만들고, 코드 전체를 한번에 리팩토링 하는 일
- 이런 작업은 보통 긴 시간을 요구하며, 그 기간동안 새로운 기능도 추가로 구현하기 어렵다.

가장 우선 해야할 일은
1. 변경의 범위를 제한하는 것
   - 단순화 될 수 있는 부분이 존재한다고 하더라도, 지금 당장 변할 필요가 없는 부분을 일단 그냥 둔다.
2. 테스트와 리팩토링 사이에 존재하는 교착상태를 해소시켜 주는 것.
   
## TDD와 패턴의 관계

패턴 주도 설계 시 구현 방법으로서의 TDD.
- 만약 strategy 패턴을 사용하기로 결정했다면
- 변화를 위한 테스트를 작성하고 메서드로 구현
- 리팩토링 단계에서 strategy 패턴을 적용하여 작성
- 패턴을 포함한 테스트 작성

하지만 완벽한 설계는 존재하지 않기에 **시스템이 무슨일을 할지**를 정하고 나중에 **설계가 알아서 정해지도록** 하는 것이 낫다.

## 어째서 TDD는 잘 작동하는가?

1. 결함 감소
   - 결함을 빨리 고칠수록 비용은 낮아진다
   - 소프트웨어 작업에 대한 신뢰가 생긴다.
2. 설계 결정에 대한 피드백 고리 단축
   - 설계 결정에 대해 바로 테스트가 가능하기에 피드백을 내리는데 시간이 적게 든다.
