...
객체나 메서드의 일부분만을 바꾸려면, 바꿔야할 부분을 격리하자.
일단 바꿀 부분을 격리하고 바꾸는 작업을 수행하면 작업을 되돌리기도 매우 수월해진다.
- 메서드가 하나 더 명시되는 비용과 하나의 개념이 명시되는 가치의 균형이 중요하다.
  - 예를 들어 findRate()라는 메서드는 단순히 인스턴스 변수의 반환이라고 한다면, 메서드 상태로 두는 것과 인라인시키고 메서드를 삭제하는 것의 균형을 잡아야한다.
- 변화를 격리하기 위한 방법
  - 메서드 추출하기
  - 객체 추출하기
  - 메서드 객체
